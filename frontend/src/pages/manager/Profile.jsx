...
export default function ManagerProfile() {
  ...
  const [gauge1Filter, setGauge1Filter] = useState(()=>({ year: new Date().getFullYear(), month: new Date().getMonth()+1 }));
  ...
  const fetchMonthlyAverage = async (y, m) => {
    try {
      const token = getToken();
      if (!userId) return { avg: 0, target: 100 };
      // Use the same endpoint as ManagerDashboard gauges
      ...
      return { avg: Math.round(avg * 100) / 100, target: 100 };
    } catch (_) {
      return { avg: 0, target: 100 };
    }
  };

  useEffect(()=>{
    const run = async () => {
      const a = await fetchMonthlyAverage(gauge1Filter.year, gauge1Filter.month);
      setGauge1Avg(a.avg); setGauge1Target(a.target);
    };
    run();
  }, [userId, gauge1Filter.year, gauge1Filter.month]);

  useEffect(()=>{
    const run = async () => {
      const a = await fetchMonthlyAverage(gauge2Filter.year, gauge2Filter.month);
      setGauge2Avg(a.avg); setGauge2Target(a.target);
    };
    run();
